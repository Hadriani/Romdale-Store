...
import PANTS_MAP from "../../../pants-data";
...
function Pants() {
    ...
    const [sortedItems, setSortedItems] = useState([...PANTS_MAP]);
    ...
    function handleSelectChange(event) {
        Router.push(`/collections/pants/${event.target.value}`)
    }

    function handleSelectChange2(event) {
        Router.push(`/collections/pants/PricePantsRange/${event.target.value}`)
    }
    ...
    const pantsToDisplay = sortedItems.slice(currentPage * 8, (currentPage + 1) * 8);
                    ...
                    {pantsToDisplay.map(({ id, name, imageUrl, price, cartButton, favoriteButton }) => (
                        <div key={id} className={styles.pantitemcontainer}
                            ...
                            <div style={{ textAlign: 'center' }}>
                                <Link href="/collections/pants/[id]" as={`/collections/pants/${id}`}>
