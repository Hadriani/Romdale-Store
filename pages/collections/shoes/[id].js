...
export async function getStaticPaths() {
    ...
    const paths = Object.keys(SHOES_MAP).map(id => ({
        params: { id },
    }));
...
export async function getStaticProps({ params }) {
    const item = SHOES_MAP[params.id];

    return { props: { item } };
}

export default function Shoe({ item }) {
        ...
        <div className={styles.container}>
            ...
            <div className={styles.idcontent}>
                ...
                <div className={styles.imagedetail}>
                    <Image src={item.imageUrl} alt={item.name} width="500" height="550" className={styles.imageid} />
                </div>
            </div>
